...
const reducer = (state, action) => {
  ...
  switch (action.type) {
    ...
    case 'SET_JSON_READY':
      return { ...state, clothesHistory: [...state.clothesHistory, ...action.payload] }
    case 'NEW_DAY':
      return { ...state, clothesHistory: [...state.clothesHistory, state.currentDay], currentDay: [] }
    default:
      return state
  }
}
...
const AllLists = ({ state }) => {
  ...
  const allClothesUsed = (state.clothesHistory.length > 0) ? removeDuplicated(state.clothesHistory.join().split(',')) : []
      ...
      {state.currentDay.map((clothesName, i) => {
        return (
          <div
            key={i}
            id={clothesName}
            className='clothes-item-current-day'
          >
            <span>{clothesName}</span>
          </div>
        )
...
const App = () => {
  ...
  const handlerTextArea = (e) => setTextArea(e.currentTarget.value)
  ...
  return (
    <div className="container">
      <div className="form-block">
        <input placeholder='What are u wearing today?' value={inputValue} onChange={handlerValue} />
